...
// A valid default Config.ini for a fresh game folder (game never run, so the engine
// hasn't written one yet). Matches the retail layout: CRLF endings, the ';' comment
// banner, and the trailing newline the engine demands ("The ENTER after the last line
// is very important"). The launcher OWNS this file — it's never shipped in the manifest.
fn default_config_ini(s: &Settings) -> Vec<u8> {
    let fs_val = if s.fullscreen { "TRUE" } else { "FALSE" };
    format!(
        ";:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;\r\n\
         ;  Legends Awakened - generated by the launcher      ;\r\n\
         ;  The ENTER after the last line is very important!! ;\r\n\
         ;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;:;\r\n\
         FileName=.\\game\\AvatarMP.vmo\r\n\
         FullScreen={fs_val}\r\n\
         Width={}\r\n\
         Height={}\r\n\
         ClipCursor=TRUE\r\n\
         DisableKeys=FALSE\r\n\
         TexturesPath=.\\game\\textures\\\r\n\
         SoundsPath= .\\game\\sounds\\\r\n",
        s.width, s.height
    ).into_bytes()
}
...
// engine demands) byte-for-byte intact. If the file doesn't exist yet (fresh
// install, game never run) we CREATE a valid default instead of failing.
fn write_config(dir: &Path, s: &Settings) -> Result<(), String> {
    let path = dir.join("Config.ini");
    let raw = match fs::read(&path) {
        Ok(r) => r,
        Err(ref e) if e.kind() == std::io::ErrorKind::NotFound =>
            return fs::write(&path, default_config_ini(s)).map_err(|e| format!("create Config.ini: {e}")),
        Err(e) => return Err(format!("read Config.ini: {e}")),
    };
    let fs_val = if s.fullscreen { "TRUE" } else { "FALSE" };
